...
const white = new Player(Team.White, [new PShape(Team.White, { x: 3, y: 2 })]);
const black = new Player(Team.Black, [new MShape(Team.White, { x: 0, y: 0 })]);
...
setInterval(() => {
  [white, black].forEach(player => {
    for (const p of player.pieces) {
      ...
      if (space && p.emptyMoves.length) player.move(board, p, space);
    }
  });
